...
-- Cached Go version (fetched once at startup, never invalidated)
local go_version_cached = (function()
  local ok, out = pcall(vim.fn.system, "go version")
  if ok and out ~= "" then
    -- "go version go1.22.5 darwin/arm64" → "go1.22.5"
    return " " .. vim.split(vim.trim(out), " ")[3] .. " "
  end
  return ""
end)()
...
local function statusline()
  ...
  -- Go version (only for Go files, uses cached value)
  local go_version = ""
  ...
  return table.concat({
    "%#StatusLineMode#", label, " ",
    "%*%#StatusFilePath#", path,
    "%*%#StatusSelCount#", sel,
    "%*%#StatusSeparator#%=%#StatusGoVersion#", go_version, "%*%#StatusLocation#", loc,
    "%*",
  })
end
...
-- Refresh on mode / cursor / buffer changes
local refresh_augroup = vim.api.nvim_create_augroup("StatuslineRefresh", { clear = true })
vim.api.nvim_create_autocmd({ "ModeChanged", "CursorMoved", "BufEnter", "WinEnter", "TextChanged", "TextChangedI" }, {
