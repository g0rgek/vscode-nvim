...
sidebar.panels = {
	...
	database = {
		...
		open = function()
			...
			if w then
				move_to_sidebar(w, sidebar.panels.database.width)
			end
		end,
	},
	history = {
		...
		open = function()
			...
			if w then
				move_to_sidebar(w, sidebar.panels.history.width)
			end
		end,
		close = function()
			local w = find_win("time-machine-list")
	...
	grpc = {
		...
		open = function()
			...
			if w then
				move_to_sidebar(w, sidebar.panels.grpc.width)
			end
		end,
		close = function()
	...
	grug = {
		...
		close = function()
			...
			local buf = vim.iter(vim.api.nvim_list_bufs()):find(function(b)
				return vim.bo[b].filetype == "grug-far" and vim.api.nvim_buf_is_valid(b)
			end)
			if buf then
